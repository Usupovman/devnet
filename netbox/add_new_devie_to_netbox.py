...
def get_loopback_ip():
    LOCAL_ENDPOINT = NETBOX_API_ROOT + "/ipam/prefixes/8/available-ips/"
    r = requests.post(LOCAL_ENDPOINT, headers=headers)
    r_json = r.json()
    loopack_ip = r_json['address']
...
def get_loopback_id(name):
    LOCAL_ENDPOINT = NETBOX_API_ROOT + "/dcim/interfaces/?device={}&name=loopback".format(name)
...
loopback_id = get_loopback_id(device_name)

add_ip_address(loopback_id, loopback_ip,name=device_name)
